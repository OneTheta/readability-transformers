...
from readability_transformers.models import TwoStepTRFPrediction, AttnFCRegression
...
def pred_twostep_train(output_path):
    ...
    train_df, valid_df = commonlit_data.stratified_split_data(ratios=(0.95, 0.05), ratio_cache_labels=("train_strat", "valid_strat"))
    ...
    twostep_trf.fit(
        ...
        train_metric=RankingMSELoss(alpha=0.5),
        evaluation_metrics=[nn.MSELoss(), RankingMSELoss(alpha=0.5)],
        batch_size=8,
        write_csv=True,
        epochs=epochs,
...
def rp_train(model, output_path):
    ...
    prediction_model = AttnFCRegression(
...
parser.add_argument('--count', default=2, help='sum the integers (default: find the max)')
parser.add_argument('--device', default="cuda:0")
parser.add_argument('--base-model', default="roberta-base")

args = parser.parse_args()

no_twostep_path = "checkpoints/eval_twostep_2/pred_twostep"

count = args.count
device = args.device
base_model = args.base_model

output_path = os.path.join(no_twostep_path, f"{base_model.replace('-', '_')}_twostep_{count}_2")
